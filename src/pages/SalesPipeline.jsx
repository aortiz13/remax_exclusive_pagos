...
import {
  ...
  fetchDealHistory, spawnDeal, PIPELINE_TRANSITIONS, deleteDeal,
...
import {
  ...
  ArrowRight, AlertTriangle, Loader2, SkipForward, Sparkles, Trash2
} from 'lucide-react'
      ...
      <AnimatePresence>
        {dealToDelete && (
          <motion.div
            ...
            onClick={() => !isDeleting && setDealToDelete(null)}
          >
            <motion.div
            ...
            >
              <div className="p-6 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Eliminar negocio</h3>
                    <p className="text-sm text-gray-500">Esta acción no se puede deshacer</p>
                  </div>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Se eliminará permanentemente el negocio de <strong>{`${dealToDelete.contact?.first_name || ''} ${dealToDelete.contact?.last_name || ''}`.trim() || 'Sin contacto'}</strong> y todo su historial de etapas.
                </p>
              </div>
              <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800 flex justify-end gap-3">
                ...
                >
                  {isDeleting ? <><Loader2 className="w-4 h-4 animate-spin" /> Eliminando...</> : <><Trash2 className="w-4 h-4" /> Eliminar</>}
                </button>
              </div>
            </motion.div>
          </motion.div>
...
function DealCard({ deal, isDragged, onDragStart, onDragEnd, onOpenDetail, onNavigateContact, onDelete }) {
    ...
    >
      ...
      <div className="px-3 py-1.5 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
        ...
        <div className="flex items-center gap-1">
          <button onClick={e => { e.stopPropagation(); onDelete && onDelete() }} onPointerDown={e => e.stopPropagation()}
            className="flex items-center gap-1 px-2 py-0.5 text-[11px] font-medium text-red-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors"
