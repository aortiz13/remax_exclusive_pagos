    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6 pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Right Column: Sidebar (Propiedades) */}
                <div className="space-y-6">
                    <div className="bg-white dark:bg-gray-900 rounded-xl border overflow-hidden sticky top-6">
                        <div className="p-3 border-b bg-gray-50 dark:bg-gray-800/50 flex justify-between items-center">
                            <h3 className="font-medium flex items-center gap-2">
                                <Home className="w-4 h-4" /> Propiedades ({relatedProperties.length})
                            </h3>
                            {isOwner && (
                                <Button
                                    variant="ghost"
                        ...
                        <div className="max-h-[500px] overflow-y-auto p-2 space-y-1">
                            {relatedProperties.length === 0 ? (
                                <div className="text-center py-4 text-sm text-muted-foreground">No hay propiedades vinculadas.</div>
                            ) : (
                                relatedProperties.map(prop => (
                                    <div
                                        key={prop.linkId || prop.id}
                                        className="p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 text-sm cursor-pointer group transition-colors"
                                        onClick={() => navigate(`/crm/property/${prop.id}`)}
                                    >
                                        <div className="flex justify-between items-start gap-2">
                                            <div className="flex-1 min-w-0">
                                                <div className="font-medium truncate group-hover:text-primary transition-colors">{prop.address}</div>
                                                <div className="text-xs text-muted-foreground truncate">{prop.commune}</div>
                                            </div>
                                            <div className="flex items-center gap-1 shrink-0">
                                                ...
                                                {isOwner && (
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
                                                        className="h-5 w-5 opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
                                                        onClick={(e) => {
                                                            e.stopPropagation()
                                        ...
                                        <div className="text-[10px] text-gray-500 mt-1">{prop.property_type}</div>
                                    </div>
                                ))
                            )}
                        </div>
